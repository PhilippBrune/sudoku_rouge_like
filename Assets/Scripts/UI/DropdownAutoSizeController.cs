...
namespace SudokuRoguelike.UI
{
    public sealed class DropdownAutoSizeController : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IPointerClickHandler, ISelectHandler, ISubmitHandler
    {
        ...
        {
            if (_dropdown == null || _template == null) return;

            var count = _dropdown.options != null ? _dropdown.options.Count : 0;
            if (count <= 0) count = 1;

            var visible = Mathf.Min(_maxVisibleItems, count);
            var height = (visible * _itemHeight) + (_padding * 2f);
            _template.sizeDelta = new Vector2(_template.sizeDelta.x, height);

            if (_scroll != null)
            {
                _scroll.vertical = count > _maxVisibleItems;
                _scroll.movementType = ScrollRect.MovementType.Clamped;
            }

            if (_content != null)
            {
                var contentHeight = (count * _itemHeight) + (_padding * 2f);
        ...
        public void OnPointerDown(PointerEventData eventData) => MarkInteraction();
        public void OnPointerClick(PointerEventData eventData) => MarkInteraction();
        public void OnPointerUp(PointerEventData eventData) => MarkInteraction();
        public void OnSelect(BaseEventData eventData) => MarkInteraction();
        public void OnSubmit(BaseEventData eventData) => MarkInteraction();

        private void OnDropdownValueChanged(int _) => MarkInteraction();

        private void MarkInteraction()
        {
            _lastInteractionUnscaledTime = Time.unscaledTime;
        }
    }
}
