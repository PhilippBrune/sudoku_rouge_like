...
|---------|------|--------|---------|
| 1.1 | 2026-04-14 | implemented | Added requirement IDs (TUTO-BASICS-TRIGGER, TUTO-BASICS-PROMPT, TUTO-BASICS-BOARD, TUTO-BASICS-STEP, TUTO-BASICS-DONE). |
| 1.0 | 2026-04-09 | implemented | TutorialModeService.BuildSudokuBasicsLevel() + CreateSudokuBasicsRunState() added. SudokuBasicsTutorialController 7-step overlay implemented. MainMenuController.ShowSudokuBasicsPrompt(), LaunchSudokuBasicsTutorial(), HasCompletedSudokuBasics(), MarkSudokuBasicsComplete() wired. GameBootstrap.Start() triggers prompt on first launch (non-profile-select path). "Replay Sudoku Basics" button added to Options panel. InRunController detects basics run (RunNumber==0) and starts the step controller. |
...
**[TUTO-BASICS-TRIGGER-001]** Triggered automatically on first launch if:
```
TutorialProgressState.CompletedKeys does NOT contain "sudoku_basics"
```

**[TUTO-BASICS-TRIGGER-002]** On any subsequent launch, the check is skipped. No dialog appears.

**[TUTO-BASICS-TRIGGER-003]** The check and prompt happen immediately after the main menu loads (not before), so the game's title screen appears first.
...
**[TUTO-BASICS-BOARD-001]** The tutorial uses a **fixed 4×4 board** with a pre-determined solution and a pre-determined set of given cells. Using a 4×4 keeps it non-threatening for players unfamiliar with Sudoku.
...
**[TUTO-BASICS-BOARD-002]** Mistakes made during this tutorial are **not penalised** — a wrong placement shows the conflict highlight but does not cost HP. The player can freely experiment.

---

## Step-by-Step Flow

**[TUTO-BASICS-STEP-001]** The tutorial progresses through 7 **named steps**. Each step shows a highlighted overlay and a text box anchored below or above the board.

**[TUTO-BASICS-STEP-002]** The player advances to the next step by pressing **"Next"** or (once input is required) by performing the action.
...
**[TUTO-BASICS-STEP-006]** > *"Tap the cell, then press 2 on the numpad. Give it a try!"*
...
**[TUTO-BASICS-DONE-001]** When the player either presses "Finish Tutorial" or completes the puzzle:
...
**[TUTO-BASICS-DONE-002]** The tutorial should be replayable. Placement in the menu:
