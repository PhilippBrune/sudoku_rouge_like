# Run of the Nine â€” Game Design Specification
...
|---------|------|--------|---------|
| 1.5 | 2026-04-07 | implemented | Game loop improvements: (1) Combo counter HUD â€” live streak label in puzzle, resets on mistake. (2) Class passive label shown in puzzle HUD. (3) Rest node replaced with 3-way choice (Heal / +Pencil / +Reroll Token). (4) Relic node replaced with 3-of-3 relic choice panel. (5) Cursed node added (Accept/Decline risk/reward). (6) Positive floor effect per floor (Bounty/LuckyItems/PencilRefill/HealingPath). (7) Boss modifier preview hint on path map nodes. (8) End-of-run score breakdown (base XP Ã— bonuses âˆ’ mistake penalty). |
| 1.4 | 2026-03-28 | implemented | Game flow: Start Game â†’ Class Select â†’ Garden Path (path overview shows before first puzzle). All submenus are full-screen opaque overlays. Locked classes display unlock conditions. 7-star requires â‰¥1 modifier. Hint/Undo features removed; Q = Save & Quit. |
| 1.3 | 2026-03-24 | implemented | Localization system (EN/DE): LocalizationService with dictionary-based translation. Language selector rebuilds all menu UI. Class select info panel moved to dedicated dark box between button grid and bottom controls. Game launch fixed: GameBootstrap.BindRunToMap() wires RunDirectorâ†’RunMapControllerâ†’first level start. |
...
| 1.1 | 2026-03-23 | implemented | Floor modifier system: floors 2â€“5 apply persistent modifiers to all puzzles (not just boss). Boss choice scaling unchanged (N from N+1). Implemented in RunDirector, BossService, and UI. |
...
2. Enter garden floor (5 floors per run) â€” a **positive floor effect** may apply to all puzzles this floor
3. Choose Calm or Risk route through branching path
4. Encounter a node â€” may be: Puzzle, Rest (3-way choice), Relic (3-of-3 choice), Shop, Elite, or Cursed (Accept/Decline)
5. Solve Sudoku puzzles under HP + Pencil constraints â€” **combo streak** tracked per correct tile
...
9. Reach boss gate â†’ preview boss hint on map â†’ choose modifiers â†’ solve boss puzzle
...
| 5â˜… | 80% | `(5+3)Ã—0.1` |
| 6â˜… | 90% | `(6+3)Ã—0.1` |
| 7â˜… | 100% | Tutorial only â€” 0 givens |

Formula: `MissingPercent = (stars + 3) Ã— 0.1`, clamped to `[0.01, 0.95]` (except 7â˜… tutorial).
...
| 1 | Alternate rectangular (rotated box dimensions) | 6Ã—6, 8Ã—8, 9Ã—9 |
| 2 | Irregular template A | 5Ã—5â€“9Ã—9 |
| 3 | Jigsaw / Irregular template B | 5Ã—5â€“9Ã—9 |

Irregular puzzles are controlled by `RunState.AllowIrregularPuzzles` toggle (default: true). When disabled, only variants 0â€“1 are used.
...
- Base HP is class-dependent (range: 7â€“14)
...
**Reward Formula:**
```
Gold = BaseDifficultyGold Ã— (1 + stars Ã— 0.2)
```

**Spend Formulas:**
- Pencil buy cost (during level): `20 + (20 Ã— timesPurchasedThisRun)`
- Reroll cost (post-level): `20 + (20 Ã— timesRerolledThisRun)`
...
|:---:|-------|:---:|:---:|:---:|---------|-----------------|
| 1 | Number Freak | 10 | 10 | 2 | Balanced â€” +1 Reroll Token | Default (unlocked) |
| 2 | Garden Monk | 14 | 5 | 1 | Every 5 correct placements â†’ +1 HP. Cannot buy Pencil mid-level. | Defeat 2 bosses |
...
| 7 | Reed Duelist | 9 | 9 | 2 | Perfect no-pencil tile bonus: +2 Pencil. Speed-focused â€” rewards clean, decisive play. | Find every unique item at least once (codex complete) |
| 8 | Quiet Cartographer | 12 | 6 | 1 | After solving a tile with no mistakes, preview the next tile's star rating and board size. Route-focused â€” rewards methodical planning. | Clear a full stage with no pencil use and no HP loss |
...
- **Relics:** Passive permanent-for-run effects. Runtime inventory uses `HeldRelics`, so players can hold multiple relics during a run. Acquired from starting relic assignment, relic choices, Elite rewards, and Run events — **not sold in the Shop**.
...
- If inventory full (4â˜…+): replace existing item or choose Nothing
...
- Two-phase curve: L1â€“15 linear, L16â€“40 accelerating. Total to L40: 16,860 XP
- Target: ~80 runs to reach Level 40
- XP formula per tile: `BaseXP(boardSize) Ã— StarMult + ModBonus + BossMult + PerfectBonus`
...
- When `ComboStreak â‰¥ 2`, a **gold label** appears in the puzzle HUD showing `"Combo Ã—N!"`.
...
- Displayed at all times during puzzle solving â€” serves as a persistent reminder.

### Rest Node â€” 3-Way Choice
...
- Intensity label: Low (runs 1â€“2), Medium (runs 3â€“5), High (runs 6â€“8), VeryHigh (runs 9+).
...
MistakePenalty = âˆ’2% per mistake, capped at âˆ’50%

FinalScore = BaseScore Ã— (1 + PerfectBonus + CursedBonus + VictoryBonus + MistakePenalty)
```
...
Run Score Breakdown
  ...
  Perfect puzzles Ã—3:  +180   (15%)
  Cursed accepted Ã—2:  +240   (20%)
  Victory bonus:       +300   (25%)
  Mistakes Ã—8:         âˆ’192   (âˆ’16%)
...
## 10) Run Structure â€” 5-Floor Garden
...
- One cross-edge per floor connecting Calm â†” Risk
- Node types: Puzzle, Elite, Shop, Relic, Event, Boss
- **Floor modifiers**: floors 2â€“5 apply persistent modifiers to **all** puzzles on that floor (not just boss). Count increases per floor (0 â†’ 1 â†’ 2 â†’ 3 â†’ 4). Rolled automatically on floor entry from full modifier pool.
- Boss gate at end of each floor with modifier choice scaling (Floor N â†’ pick N from N+1). Boss puzzles stack floor modifiers + boss-chosen modifiers.
- Floor transition: Garden Cleared card â†’ cutscene â†’ fade-in

### Floor Themes

| Floor | Theme | Grid Size Range |
|:---:|-------|:---:|
| 1 | Bamboo Courtyard | 5Ã—5â€“6Ã—6 |
| 2 | Moss Garden | 6Ã—6â€“7Ã—7 |
| 3 | Koi Terrace | 7Ã—7â€“8Ã—8 |
| 4 | Stone Lantern Walk | 8Ã—8â€“9Ã—9 |
| 5 | Shrine Summit | 9Ã—9 |
...
- Intensity scales with run number: Low (runs 1â€“2) â†’ VeryHigh (runs 9+)
- Modifier legend panel with tap-to-isolate for visual clarity
- Puzzles are not required to be logically solvable â€” items and relics compensate

---

## 12) Tutorial Mode

See [TutorialModeSystem](TutorialModeSystem.md) for sandbox configuration, modifier toggles, and 7â˜… rules.

### Summary

- Isolated sandbox with no progression (no XP, no gold, no unlocks)
- Player configures: board size (5â€“9), star difficulty (1â€“7), region variant, modifier toggles
- 7â˜… = 100% missing (0 givens, tutorial only)
...
- Depth â‰¥ 10: 2 random modifiers per level
- No item drops, minimal gold
- XP leaderboard focus

### Spirit Trials

- Fixed 9Ã—9, baseline 3â˜…
- Timer active, no gold, limited items
- Score: `BasePoints Ã— SpeedMultiplier Ã— ConstraintBonus âˆ’ MistakePenalty`
...
- Puzzles not required to be logically solvable â€” items/relics compensate
...
|--------|-------|-----------|-----------------|--------|
| REQ-GENERAL-013 | TITLE | `â€”` | â€” | âœ— missing |
| REQ-GENERAL-014 | RunStartSequenceSystem | `Assets\Scripts\Run\RunDirector.cs` | RunDirector | âœ“ verified |
| REQ-GENERAL-015 | ClassSelectionScreen | `â€”` | â€” | âœ— missing |
| REQ-GENERAL-016 | TITLE | `â€”` | â€” | âœ— missing |
| REQ-GENERAL-017 | GameStateController | `Assets\Scripts\Run\RunDirector.cs` | RunDirector | âœ“ verified |
| REQ-GENERAL-018 | FloorSelectionScreen | `Assets\Scripts\UI\MainMenuController.cs` | â€” | âœ“ verified |
| REQ-GENERAL-019 | CoreLoopSudokuPuzzle | `Assets\Scripts\Run\RunDirector.cs` | â€” | âœ“ verified |
| REQ-GENERAL-020 | HpUnitsResource | `Assets\Scripts\Economy\RelicService.cs` | â€” | âœ“ verified |
| REQ-GENERAL-021 | PencilUnitsResource | `Assets\Scripts\Run\RunDirector.cs` | â€” | âœ“ verified |
| REQ-GENERAL-022 | GoldResource | `â€”` | â€” | âœ— missing |
| REQ-GENERAL-023 | ItemAndRelicManagement | `Assets\Scripts\Run\RunDirector.cs` | â€” | âœ“ verified |
| REQ-GENERAL-024 | XpProgressionSystem | `Assets\Scripts\Economy\XpService.cs` | â€” | âœ“ verified |
| REQ-GENERAL-025 | PostLevelItemRollPhase | `Assets\Scripts\Run\RunDirector.cs` | â€” | âœ“ verified |
| REQ-GENERAL-026 | CoreLoopBosses | `Assets\Scripts\Run\SpiritTrialsService.cs` | â€” | âœ“ verified |
| REQ-GENERAL-027 | PathSystem | `Assets\Scripts\Run\RunDirector.cs` | RunDirector | âœ“ verified |
| REQ-GENERAL-028 | ClassXpProgressionSystem | `Assets\Scripts\UI\MainMenuController.cs` | â€” | âœ“ verified |
| REQ-GENERAL-029 | UiController | `Assets\Scripts\Run\RunDirector.cs` | RunDirector | âœ“ verified |
