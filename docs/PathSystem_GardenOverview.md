...
|---------|------|--------|---------|
| 1.5 | 2026-04-14 | implemented | Added requirement IDs (MAP-STRUCT, MAP-ROUTE, MAP-CROSS, MAP-NODE, MAP-FX, MAP-ICON, MAP-UI). |
...
**[MAP-STRUCT-001]** The Garden Overview screen is the central navigation hub of a run. The player walks through a **Japanese garden** — each garden represents one **floor** of the run. There are **5 floors** in total. Each floor is a self-contained garden with its own visual theme, background art, and music.

**[MAP-STRUCT-002]** Within each floor, the player navigates a branching path from a **shared start tile** through either the **Calm Route** or the **Risk Route**, ultimately arriving at a **shared Boss Gate tile**. Defeating the boss advances the player to the next floor.
...
**[MAP-ROUTE-001]** Calm Route characteristics:
...
**[MAP-ROUTE-002]** Risk Route characteristics:
...
**[MAP-CROSS-001]** Each floor has exactly **one cross-connection** linking one tile on the Calm Route to one tile on the Risk Route.
...
### Cursed
**[MAP-NODE-CURSED-001]** An opt-in risk/reward node. The player is presented with an **Accept / Decline** panel before the puzzle loads.
- **[MAP-NODE-CURSED-002] Accept**: puzzle runs with `IsCursed = true` — applies **×1.5 gold multiplier** and **×1.5 XP multiplier**, and adds **one extra random modifier** (drawn from the 15-modifier pool, stacked on any existing floor modifiers).
- **[MAP-NODE-CURSED-003] Decline**: a standard puzzle of the same board size/star rating starts with no extra modifiers or multipliers.
...
- **[MAP-NODE-CURSED-004]** Accepted cursed puzzles are tracked for the run score breakdown (`CursedPuzzlesAccepted`).
...
**[MAP-FX-001]** Each floor rolls **one random positive effect** (or None) when the player enters it. The effect applies to **all puzzles on that floor** — normal, elite, and boss.
...
**[MAP-FX-002]** Effect is rolled seeded from `RunState.Seed + floorIndex` for determinism.
**[MAP-FX-003]** Stored in `RunState.HasPositiveFloorEffect` + `ActivePositiveFloorEffect`.
**[MAP-FX-004]** Shown on the path screen as a banner (e.g. `"★ Bounty Floor (+40% gold)"`), returned by `RunMapController.GetPositiveFloorEffectLabel()`.
**[MAP-FX-005]** `None` has equal probability to each named effect (i.e. 20% each).
