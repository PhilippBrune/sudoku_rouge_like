...
When the player reaches a **Relic tile node** on the garden path:

1. A single random relic is generated (tier based on floor weight table + Risk route bonus if applicable).
2. **If the player has no relic:** The relic is granted immediately with a brief reveal animation.
3. **If the player already holds a relic:** A choice panel appears showing:
   - **Left:** Currently held relic (name, tier, effect description)
   - **Right:** Newly offered relic (name, tier, effect description)
   - Player selects one to keep. The other is discarded.
4. The path overview resumes after the choice.
